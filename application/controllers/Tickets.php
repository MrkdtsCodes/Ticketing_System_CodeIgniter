    ...
    class Tickets extends CI_Controller
    {
        ...
        public function refactoredTckttables()
        {
            ...
            $config['base_url'] = 'http://localhost/Projects/TICKETING_SYSTEM/';
        ...
        public function pagination_pages($lims, $offs)
        {
            $data = $this->Tickets_Model->paginations($lims, $offs);
        ...
        //      $data = $this->Tickets_Model->();
        // }

        public function controller_for_batch_update()
        {
            ...
            // $data = ['adxyz' => ['name' => 'sadfsdfsd', 'email' => ..., ...], 'abcd' => [...]]

            if (empty($data)) {
                $this->output
                    ->set_status_header(400)
                    ->set_content_type('application/json')
                    ->set_output(json_encode(['status' => 'error', 'message' => 'No data received']));
                return;
            }

            foreach ($data as $student_id => $student) {
                // save/update each student row here
            }
        ...
        public function applyTicketFilters($limit = 0, $stats = 'on going'){
            $post = [];
            // $post =[
            //     'limit' => 15
            // ];
            ...
            $data['crtdTickets'] = $this->Tickets_Model->get_many_by($limit);
        ...
        public function filter_tickets(){
            ...
            if(!empty($post['formObjct']['status'])){
                $params['f_status'] = $post['formObjct']['status'];
            }
